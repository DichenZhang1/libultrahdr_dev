...
class UhdrUnCompressedStructWrapper {
 public:
  ...
  uhdr_uncompressed_ptr getImageHandle();
...
class UhdrCompressedStructWrapper {
 public:
  ...
  bool allocateMemory();
  uhdr_compressed_ptr getImageHandle();
...
uhdr_uncompressed_ptr UhdrUnCompressedStructWrapper::getImageHandle() { return &mImg; }
...
uhdr_compressed_ptr UhdrCompressedStructWrapper::getImageHandle() { return &mImg; }
...
void decodeJpegRImg(uhdr_compressed_ptr img, [[maybe_unused]] const char* outFileName) {
  jpegr_info_struct info{};
  JpegR jpegHdr;
  ASSERT_EQ(ULTRAHDR_NO_ERROR, jpegHdr.getJPEGRInfo(img, &info));
  ...
  ASSERT_EQ(ULTRAHDR_NO_ERROR, jpegHdr.decodeJPEGR(img, &destImage));
...
TEST(JpegRTest, DecodeAPIWithInvalidArgs) {
  ...
  // test jpegr image
  ASSERT_NE(uHdrLib.decodeJPEGR(nullptr, &destImage), ULTRAHDR_NO_ERROR)
      << "fail, API allows nullptr for jpegr img";
  ASSERT_NE(uHdrLib.decodeJPEGR(jpgImg.getImageHandle(), &destImage), ULTRAHDR_NO_ERROR)
      << "fail, API allows nullptr for jpegr img";
  ASSERT_TRUE(jpgImg.allocateMemory());

  // test dest image
  ASSERT_NE(uHdrLib.decodeJPEGR(jpgImg.getImageHandle(), nullptr), ULTRAHDR_NO_ERROR)
  ...
  ASSERT_NE(uHdrLib.decodeJPEGR(jpgImg.getImageHandle(), &destImage, 0.5), ULTRAHDR_NO_ERROR)
...
class JpegRBenchmark : public JpegR {
 public:
  void BenchmarkGenerateGainMap(uhdr_uncompressed_ptr yuv420Image, uhdr_uncompressed_ptr p010Image,
                                ultrahdr_metadata_ptr metadata, uhdr_uncompressed_ptr map);
  void BenchmarkApplyGainMap(uhdr_uncompressed_ptr yuv420Image, uhdr_uncompressed_ptr map,
                             ultrahdr_metadata_ptr metadata, uhdr_uncompressed_ptr dest);

 private:
  const int kProfileCount = 10;
};

void JpegRBenchmark::BenchmarkGenerateGainMap(uhdr_uncompressed_ptr yuv420Image,
                                              uhdr_uncompressed_ptr p010Image,
                                              ultrahdr_metadata_ptr metadata,
                                              uhdr_uncompressed_ptr map) {
...
void JpegRBenchmark::BenchmarkApplyGainMap(uhdr_uncompressed_ptr yuv420Image, uhdr_uncompressed_ptr map,
                                           ultrahdr_metadata_ptr metadata,
                                           uhdr_uncompressed_ptr dest) {
  Profiler profileRecMap;
  profileRecMap.timerStart();
  for (auto i = 0; i < kProfileCount; i++) {
    ASSERT_EQ(ULTRAHDR_NO_ERROR, applyGainMap(yuv420Image, map, metadata, ULTRAHDR_OUTPUT_HDR_HLG,
